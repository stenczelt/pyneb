...
def assertion_statement(expected, varname, got):
    return "Assertion failed - expected '{}' to be one of: {}. Got {} instead.".format(
        varname, expected, got
    )
...
class supercell:
    ...
    def __init__(self, fast=False):
        ...
        self.set_methods = {
            ...
            "charge": "set_charge",
            "name": "set_name",
            "bulkmodulus": "set_bulkmodulus",
            "spacegroup": "set_spacegroup",
            "edensity": "set_edensity",
            "files": "set_files",
    ...
    def set_species(self, in_species):
        ...
        if not self.fast:
            ...
            assert all(isinstance(_atom, str) for _atom in in_species), self.errors[
                "E3"
            ]
    ...
    def set_name(self, in_name):
        ...
        for obj in gc.get_objects():
            if isinstance(obj, supercell) and (
                type(getattr(obj, "name")) is not type(None)
            ):
                stack_names += [getattr(obj, "name")]
        if not self.fast:
            assert (
                in_name not in stack_names
            ), "{} structure name is already in use: {}".format(in_name, stack_names)
    ...
    def set_spacegroup(self, in_spacegroups):
        ...
        if not self.fast:
            ...
            assert all(
                _convention in supported_conventions for _convention in in_spacegroups
            ), self.errors["E0"]
    ...
    def set_edensity(self, in_edensity):
        ...
        if isinstance(
            in_edensity, list
        ):  # assumes that in_edensity are given as a list of dicts
            self.edensity = {
                "xyz": np.array(
                    [[v["x"], v["y"], v["z"]] for v in in_edensity], dtype=float
                ),
                "density": np.array([v["density"] for v in in_edensity]),
            }
        elif isinstance(
            in_edensity, dict
        ):  # assumes that in_edensity is given as a dict with keys "xyz" and "density" with np.ndarray values
            assert (
                "xyz" in in_edensity and "density" in in_edensity
            ), "Assertion failed - expected 'xyz' and 'density' as keys, got {} instead!".format(
                in_edensity.keys()
            )
            assert len(in_edensity["xyz"]) == len(
                in_edensity["density"]
            ), "Assertion failed - given number of positions doesn't match the number of densities ({}!={})!".format(
                len(in_edensity["xyz"]), len(in_edensity["density"])
            )
            self.edensity = {
                "xyz": np.array(in_edensity["xyz"]),
                "density": np.array(in_edensity["density"]),
            }
        else:
            print(
                "Error - expected in_edensity to be either of list or dict, got {} instead!".format(
                    type(in_edensity)
                )
            )
            raise
    ...
    def set_files(self, in_files):
        ...
        if not self.fast:
            assert (
                self.files is None
            ), "Assertion failed - self.files already set to {} of type {}.".format(
                self.files, type(self.files)
            )
            assert isinstance(in_files, list), assertion_statement(
                list, "in_files", in_files
            )
            assert all(isinstance(_a, str) for _a in in_files), assertion_statement(
                "list of str", "in_files", in_files
            )
    ...
    def __setitem__(self, key, value):
        assert (
            key in self.iproperties
        ), "Assertion failed - got unexpected key {}. Expected one of: {}".format(
            key, self.iproperties
        )
    ...
    def ase_format(self):
        ...
        # assertions
        assert (
            self.cell is not True
        ), "Assertion failed - cell vectors have not been set yet"
        ...
        for i, _pos in enumerate(self.positions):
            ...
            atom_list.append(
                Atom(symbol=_element, position=_xyztuple, mass=_mass, charge=_charge)
            )
